...
"""Norway Enricher - Scrape emails from Norwegian company websites"""
import sys
...
SOURCE = Path("/mnt/hdd/SCRAPER_DATA/csv/EURES/Norway/Norway_contacts_50.csv")
OUTPUT_DIR = Path("/opt/ACTIVE/OPENDATA/DATA/ENRICHED")
OUTPUT = OUTPUT_DIR / "NO_ENRICHED.csv"
CACHE = OUTPUT_DIR / "no_domain_cache.json"
...
logging.basicConfig(level=logging.INFO, format="%(asctime)s - %(message)s",
    handlers=[logging.StreamHandler(), logging.FileHandler(LOG_DIR / f"no_{datetime.now():%Y%m%d}.log")])
...
def find_emails(url):
    ...
    paths = ["", "/kontakt", "/contact", "/om-oss", "/about"]
