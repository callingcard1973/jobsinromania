#!/usr/bin/env python3
"""MIVRomania.online Brevo Automation - with anti-spam protection."""
import sys
...
SENDER_RULES = SenderRules("MIVROMANIA_ONLINE")
...
SENDER_EMAIL = "office@mivromania.online"
SENDER_NAME = "MIV Romania"
DAILY_LIMIT = 290

ANOFM_DIR = Path("/mnt/hdd/SCRAPER_DATA/csv/ANOFM")

# ANOFM Domeniul activitate filter (exact match) - HoReCa and Tourism
ANOFM_SECTORS = [
    'RESTAURANTE',
    'Turism / Alimentatie',
]

TEMPLATE_RO = Path("/opt/ACTIVE/EMAIL/CAMPAIGNS/MIVROMANIA_ONLINE/templates/01_romanian.txt")
STATE_FILE = Path("/opt/ACTIVE/OPENDATA/DATA/brevo_state/MIVROMANIA_ONLINE_state.json")
STATE_FILE.parent.mkdir(exist_ok=True)

from global_sent_tracker import GlobalSentTracker
TRACKER = GlobalSentTracker()

DEFAULT_SUBJECT = "Personal calificat HoReCa disponibil imediat"
DEFAULT_BODY = """Buna ziua,

Avem disponibili muncitori calificati pentru sectorul HoReCa: bucatari, ospatari, barmani, receptioneri.

Personal din Romania, Nepal, Sri Lanka si Kenya, cu experienta si documente in regula.

Daca aveti nevoie de personal, va rog sa raspundeti si va trimit lista cu candidatii disponibili.

Cu stima,
Tudor Seicarescu
office@mivromania.online
"""
        ...
        with open(anofm_file, 'r', encoding='utf-8', errors='ignore') as f:
            for row in csv.DictReader(f):
                ...
                if domain in FREE_EMAIL_DOMAINS:
                    continue
                can_send, _ = TRACKER.can_send(email, campaign="MIVROMANIA_ONLINE")
                if not can_send:
                    continue
...
def send_batch(dry_run=False):
    print(f"\n=== MIVROMANIA.ONLINE - {datetime.now().strftime('%Y-%m-%d %H:%M')} ===")
    state = load_state()
    ...
    print(f"HoReCa ANOFM: {len(contacts)} unsent")
    if not contacts:
    ...
    sender = BrevoSafeSender(API_KEY, SENDER_EMAIL, SENDER_NAME, campaign_name="MIVROMANIA_ONLINE", batch_size=1, wait_minutes=4, dry_run=dry_run)
    def on_sent(contact, success, msg):
        if success:
            state["sent"].append(contact["email"])
            TRACKER.mark_sent(contact["email"], campaign="MIVROMANIA_ONLINE", sender="brevo_mivromania_online")
    try:
...
def show_status():
    ...
    print(f"\n=== MIVROMANIA.ONLINE STATUS ===")
    print(f"Sectors: HoReCa, Tourism")
