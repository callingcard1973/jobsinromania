#!/usr/bin/env python3
"""Careworkers.eu Brevo Automation - Healthcare ANOFM contacts."""
import sys
...
load_dotenv('/opt/ACTIVE/EMAIL/CAMPAIGNS/CAREWORKERS/.env')

# CLAUDE.md compliance: Pre-send validation
SENDER_RULES = SenderRules("CAREWORKERS")
...
SENDER_NAME = "Care Workers EU"
DAILY_LIMIT = 290

ANOFM_DIR = Path("/mnt/hdd/SCRAPER_DATA/csv/ANOFM")

# ANOFM Domeniul activitate filter (exact match)
# Expanded to include home care workers
ANOFM_SECTORS = [
    'Medicina / Sanatate /Psihoterapie',  # Healthcare
    'ASISTENTA SOCIALA',                   # Social assistance
    'Au pair / Babysitter / Curatenie',    # Home care workers
]

TEMPLATE_FILE = Path("/opt/ACTIVE/EMAIL/CAMPAIGNS/CAREWORKERS/templates/template_romania.txt")
STATE_FILE = Path("/opt/ACTIVE/OPENDATA/DATA/brevo_state/CAREWORKERS_state.json")
STATE_FILE.parent.mkdir(exist_ok=True)

from global_sent_tracker import GlobalSentTracker
TRACKER = GlobalSentTracker()

DEFAULT_SUBJECT = "Personal medical disponibil pentru 2026"
DEFAULT_BODY = """Buna ziua,

Avem disponibili asistenti medicali, infirmieri si ingrijitori din Romania, Republica Moldova, Nepal si Sri Lanka.

Personal cu experienta, documente in regula, disponibili pentru contracte pe termen lung.

Daca aveti nevoie de personal medical, va rog sa raspundeti si va trimit lista cu candidatii disponibili.
...
def load_template():
    if TEMPLATE_FILE.exists():
        ...
        subject = lines[0].replace("Subject:", "").replace("SUBIECT:", "").strip()
        body = '\n'.join(lines[2:])
        return subject, body
        ...
        with open(anofm_file, 'r', encoding='utf-8', errors='ignore') as f:
            for row in csv.DictReader(f):
                ...
                if domain in FREE_EMAIL_DOMAINS:
                    continue
                can_send, _ = TRACKER.can_send(email, campaign="CAREWORKERS")
                if not can_send:
                    continue
...
def send_batch(dry_run=False):
    print(f"\n=== CAREWORKERS.EU - {datetime.now().strftime('%Y-%m-%d %H:%M')} ===")
    state = load_state()
    ...
    print(f"Healthcare ANOFM: {len(contacts)} unsent")
    if not contacts:
    ...
    sender = BrevoSafeSender(API_KEY, SENDER_EMAIL, SENDER_NAME, campaign_name="CAREWORKERS", batch_size=1, wait_minutes=4, dry_run=dry_run)
    def on_sent(contact, success, msg):
        if success:
            state["sent"].append(contact["email"])
            TRACKER.mark_sent(contact["email"], campaign="CAREWORKERS", sender="brevo_careworkers")
    try:
...
def show_status():
    ...
    print(f"\n=== CAREWORKERS.EU STATUS ===")
    print(f"Sectors: {ANOFM_SECTORS}")
