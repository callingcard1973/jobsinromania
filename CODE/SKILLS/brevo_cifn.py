#!/usr/bin/env python3
"""CIFN.info Brevo Automation - Transport & Security ANOFM contacts."""
import sys
...
load_dotenv('/opt/ACTIVE/EMAIL/CAMPAIGNS/CIFN/.env')

# CLAUDE.md compliance: Pre-send validation
SENDER_RULES = SenderRules("CIFN")
...
ANOFM_SECTORS = [
    'Servicii transport / curierat',
    'Armata / Paza/ Protectie',
    'SERVICII DE PAZA SI PROTECTIE'
]

TEMPLATE_DIR = Path("/opt/ACTIVE/EMAIL/CAMPAIGNS/CIFN/templates")
STATE_FILE = Path("/opt/ACTIVE/OPENDATA/DATA/brevo_state/CIFN_state.json")
STATE_FILE.parent.mkdir(exist_ok=True)

from global_sent_tracker import GlobalSentTracker
TRACKER = GlobalSentTracker()

DEFAULT_SUBJECT = "Personal pentru transport si paza disponibil 2026"
DEFAULT_BODY = """Buna ziua,

Avem disponibili soferi, curieri si agenti de paza din Romania, Republica Moldova, Nepal si Sri Lanka.
        ...
        with open(anofm_file, 'r', encoding='utf-8', errors='ignore') as f:
            for row in csv.DictReader(f):
                ...
                if domain in FREE_EMAIL_DOMAINS:
                    continue
                can_send, _ = TRACKER.can_send(email, campaign="CIFN")
                if not can_send:
                    continue
...
def send_batch(dry_run=False):
    print(f"\n=== CIFN.INFO - {datetime.now().strftime('%Y-%m-%d %H:%M')} ===")
    state = load_state()
    ...
    print(f"Transport/Security ANOFM: {len(contacts)} unsent")
    if not contacts:
    ...
    sender = BrevoSafeSender(API_KEY, SENDER_EMAIL, SENDER_NAME, campaign_name="CIFN", batch_size=1, wait_minutes=4, dry_run=dry_run)
    def on_sent(contact, success, msg):
        if success:
            state["sent"].append(contact["email"])
            TRACKER.mark_sent(contact["email"], campaign="CIFN", sender="brevo_cifn")
    try:
