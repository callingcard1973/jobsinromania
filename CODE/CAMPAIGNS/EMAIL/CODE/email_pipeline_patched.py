...
# Spam folder names by provider (tried in order)
SPAM_FOLDERS = ["[Gmail]/Spam", "Junk", "INBOX.spam", "INBOX.Junk", "Spam", "Bulk Mail", "Bulk"]
...
def _detect_spam_folder(imap):
    """Try to find the spam/junk folder for this IMAP account."""
    try:
        _, folders = imap.list()
        available = []
        for f in (folders or []):
            decoded = f.decode(errors="replace")
            # Extract folder name from IMAP LIST response: (* (\\Junk ...) "/" "FolderName")
            m = re.search(r'"([^"]*)"$', decoded) or re.search(r'\s(\S+)$', decoded)
            if m: available.append(m.group(1))
        for candidate in SPAM_FOLDERS:
...
def process(acct_filter, state, dry):
    ...
    stats = {"orders":0,"applicants":0,"contacts":0,"dnc":0,"skipped":0,"spam_rescued":0}
    for em, pw, srv, label in accounts:
        ...
        # Detect spam folder before processing
        spam_folder = _detect_spam_folder(imap)
        folders_to_scan = ["INBOX"]
        if spam_folder:
            folders_to_scan.append(spam_folder)
        for folder in folders_to_scan:
            is_spam = folder != "INBOX"
            st, _ = imap.select(folder, readonly=True)  # always readonly to avoid marking as read
            if st != "OK":
                if is_spam: log(f"  Spam folder '{folder}': select failed, skipping")
                continue
            if is_spam: log(f"  + Scanning spam folder: {folder}")
            ...
            for mid in all_ids:
                ...
                if BOUNCE_SENDERS.search(frm) or BOUNCE_SUBJ.search(subj):
                    ...
                    if bounced and bounced.endswith(PERSONAL_DOMAINS):
                        ...
                        if not is_spam: to_delete.append(mid)
                    else:
                        stats["skipped"]+=1
                    if is_spam: stats["spam_rescued"]+=1
            ...
            # Delete bounce messages for non-existent personal emails (INBOX only)
            if to_delete and not dry and not is_spam:
                imap.select("INBOX", readonly=False)  # reopen read-write for deletion
