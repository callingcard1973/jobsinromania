#!/usr/bin/env python3
"""Unified email pipeline: scan mailboxes → classify → route. Applicants → elena APPLICATIONS_RECEIVED.
Usage: python3 email_pipeline.py [--dry-run] [--reprocess] [--accounts gmail] [--no-llm]"""
import imaplib, email, json, csv, os, sys, ssl, re, pickle, requests, codecs
try: codecs.lookup("unknown-8bit")
except LookupError: codecs.register(lambda n: codecs.lookup("utf-8") if n == "unknown-8bit" else None)
...
_cc = [os.environ.get("CLF_PATH",""), "/opt/ACTIVE/EMAIL/ORDERS/models/email_classifier.pkl", str(DIR/"models/email_classifier.pkl")]
CLF_PATH = next((Path(p) for p in _cc if p and Path(p).exists()), Path("email_classifier.pkl"))
LLM_ENDPOINTS = [
    ("http://192.168.100.25:1234/v1/chat/completions", "qwen2.5-coder-1.5b-instruct"),  # laptop LM Studio
    ("http://localhost:11434/v1/chat/completions", "qwen2.5:1.5b"),  # raspibig Ollama
]
ELENA_EMAIL = "elena.manpower.dristor@gmail.com"
ELENA_PASSWORD = os.environ.get("GMAIL_ELENA_PASSWORD", "wmfnpikkcierkmrq")
ELENA_LABEL = "APPLICATIONS_RECEIVED"

ORDER_COLS = ["Timestamp","Clasificare","Denumire companie","Nume persoană de contact","Telefon","Email","Localitate","Număr persoane","Tip poziții","Observații","Sursa","Account"]
...
DB_HOST, DB_AVAILABLE = os.environ.get("PGHOST", "localhost"), None
PROMPT = ('Extract JSON from email. Return ONLY valid JSON.\n'
    '{"type":"","company":"","contact":"","phone":"","location":"","workers":"","positions":"","notes":""}\n'
    'type: COMANDA (needs workers now), INTERESAT (interested), PARTENER (agency), SKIP (not order).\n'
    'Extract phone from signature. workers=number only.\nSENDER: <<<S>>>  SUBJECT: <<<J>>>  EMAIL: <<<B>>>\nJSON:')

_clf, _llm_url, _llm_model, _elena_imap = None, None, None, None
...
def _find_llm():
    ...
    if _llm_url: return True
    for url, model in LLM_ENDPOINTS:
        try:
            r = requests.post(url, json={"model":model,"messages":[{"role":"user","content":"ok"}],"max_tokens":3}, timeout=5)
            if r.ok and "choices" in r.json():
                _llm_url, _llm_model = url, model
                log(f"  LLM: {model} @ {url.split('//')[1].split('/')[0]}"); return True
        except: pass
    log("  LLM: all endpoints offline"); return False

def llm_extract(body, sender, subj):
    if "--no-llm" in sys.argv or not _find_llm(): return None
    p = PROMPT.replace("<<<B>>>",body[:800]).replace("<<<S>>>",sender).replace("<<<J>>>",subj)
    try:
        r = requests.post(_llm_url, json={"model":_llm_model,"messages":[{"role":"user","content":p}],"temperature":0.1,"max_tokens":150}, timeout=120)
        m = re.search(r"\{[^{}]*\}", r.json()["choices"][0]["message"]["content"], re.DOTALL)
        return json.loads(m.group()) if m else None
    except: return None

def forward_to_elena(raw_msg, subj, se, dry):
    global _elena_imap
    if dry: return
    try:
        if _elena_imap is None:
            _elena_imap = imaplib.IMAP4_SSL("imap.gmail.com", 993, ssl_context=ssl.create_default_context())
            _elena_imap.login(ELENA_EMAIL, ELENA_PASSWORD)
            try: _elena_imap.create(ELENA_LABEL)
            except: pass
            log(f"  Elena IMAP: connected")
        _elena_imap.append(ELENA_LABEL, None, None, raw_msg)
        log(f"    -> FWD elena [{subj[:40]}] {se}")
    except Exception as e:
        log(f"    -> FWD elena FAIL: {e}"); _elena_imap = None
...
def _dp(p):
    pl=p.get_payload(decode=True); return pl.decode(p.get_content_charset() or "utf-8",errors="replace") if pl else None
def get_body(msg):
    if msg.is_multipart():
...
def get_phone(body):
    ph=re.findall(r"(?:\+?\d[\d\s\-]{8,15})",body[:1500]); return ph[0].strip() if ph else ""
...
def save_order(ext, se, frm, subj, intent, acct, dry):
    ...
    row = {"Timestamp":f"{datetime.now():%d/%m/%Y %H:%M:%S}","Clasificare":clf,"Denumire companie":ext.get("company",""),
        "Nume persoană de contact":ext.get("contact",""),"Telefon":ext.get("phone",""),"Email":ext.get("email","") or se,
        "Localitate":ext.get("location",""),"Număr persoane":ext.get("workers",""),"Tip poziții":ext.get("positions",""),
        "Observații":ext.get("notes",""),"Sursa":f"[{intent}]","Account":acct}
...
def save_applicant(se, frm, subj, body, acct, dry, raw_msg=None):
    cty, lang = "", "EN"
    ...
    row = {"Timestamp":f"{datetime.now():%d/%m/%Y %H:%M:%S}","Name":get_name(frm),"Email":se,
        "Phone":get_phone(body),"Country":cty,"Language":lang,"Subject":subj[:100],"Account":acct}
    if not dry: append_csv(APPLICANTS_F, APPL_COLS, row)
    if raw_msg: forward_to_elena(raw_msg, subj, se, dry)
...
def save_handover(se, frm, body, acct, dry):
    ...
    if not dry: append_csv(HANDOVER_F, HANDOVER_COLS, {"Timestamp":f"{datetime.now():%d/%m/%Y %H:%M:%S}",
        "Old Contact":get_name(frm),"Old Email":se,"New Contact":nn,"New Email":ne,"Company":"","Account":acct})
    save_contact(ne, f"{nn} <{ne}>", body, "handover-new", acct, dry)
    log(f"    -> {'[DRY]' if dry else 'HANDOVER'} {se} -> {nn} <{ne}>"); return True
...
def save_dnc(addr, reason, acct, dry):
    ...
    import psycopg2
    for db, sql in [("interjob_master","INSERT INTO dnc_list(email,reason) VALUES(%s,%s) ON CONFLICT DO NOTHING"),
        ("email_sender","INSERT INTO dnc_emails(email,reason,source) VALUES(%s,%s,'email_pipeline') ON CONFLICT DO NOTHING")]:
        try: c=psycopg2.connect(host=DB_HOST,database=db,user="tudor",password="tudor"); cur=c.cursor(); cur.execute(sql,(addr,reason)); c.commit(); cur.close(); c.close()
        except: pass
...
def process(acct_filter, state, dry):
    ...
    stats = {"orders":0,"applicants":0,"contacts":0,"dnc":0,"skipped":0,"fwd_elena":0}
    for em, pw, srv, label in accounts:
        ...
        imap.select("INBOX", readonly=dry)  # writable if not dry (for deletion)
        ...
        for mid in all_ids:
            ...
            se = get_email(frm)
            # noreply@ from our own domains = website form application, NOT junk
            se_domain = se.split("@",1)[1] if "@" in se else ""
            is_own_noreply = se.startswith("noreply@") and se_domain in OWN_DOMAINS
            # Skip + delete system junk (but NOT our own noreply)
            if not is_own_noreply and (any(s in se.lower() for s in SKIP_SENDERS) or SKIP_SUBJ.search(subj)):
            ...
            _, fd = imap.fetch(mid, "(RFC822)")
            raw_bytes = fd[0][1]
            msg_parsed = email.message_from_bytes(raw_bytes)
            body = get_body(msg_parsed)
            if not body.strip(): processed.add(msgid) if not dry else None; continue
            # Route: website form apps first
            if is_own_noreply or SITE_APP_SUBJ.search(subj):
                save_applicant(se, frm, subj, body, label, dry, raw_bytes)
                stats["applicants"]+=1; stats["fwd_elena"]+=1
                to_delete.append(mid)  # clean form notification from inbox
            elif UNSUB_BODY.search(body[:500]):
        ...
        # Delete autoresponders, bounces, unsubscribes from inbox
        if to_delete and not dry:
            for mid in to_delete: imap.store(mid, '+FLAGS', '\\Deleted')
            imap.expunge()
            log(f"  {label}: deleted {len(to_delete)} auto/bounce msgs")
...
def main():
    ...
    if not dry: state["last_run"]=datetime.now(timezone.utc).isoformat(); STATE_F.write_text(json.dumps(state,indent=2,ensure_ascii=False),encoding="utf-8")
    if _elena_imap:
        try: _elena_imap.logout()
        except: pass
    log(f"Done: {stats}")
