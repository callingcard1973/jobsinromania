...
DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASS = "localhost", 5432, "interjob_master", "tudor", "REDACTED"
...
def newsletter_block(lang: str) -> str:
    texts = {
        "ro": ("Abonează-te la newsletter-ul InterJob.ro și fii primul care află cele mai bune locuri de muncă din România și Europa.",
                "Abonează-te gratuit", "📩 Primești zilnic ofertele noi pe email"),
        "en": ("Subscribe to InterJob.ro newsletter and be the first to know about new job openings in Romania and Europe.",
                "Subscribe for free", "📩 Get daily job alerts by email"),
    }
    body, cta, h3 = texts.get(lang, texts["en"])
    return ('<div style="background:#f0f7ff;border:2px solid #0073aa;border-radius:8px;padding:20px;margin:30px 0;text-align:center">'
            f'<h3 style="margin-top:0;color:#0073aa">{h3}</h3>'
            f'<p style="margin:8px 0">{body}</p>'
            f'<a href="{APPLY_URL}" style="background:#0073aa;color:white;padding:10px 22px;text-decoration:none;border-radius:4px;display:inline-block;margin-top:8px">{cta}</a></div>')
...
def build_en(today_str_en: str, anofm_total: int, by_sector: Dict, count_sector: Dict,
    ...
    L.append(f"<hr><h2>{h2_ro}</h2>")
    # Batch all titles for a single translate call
    top_sectors = sorted(count_sector.items(), key=lambda x: x[1], reverse=True)[:7]
    all_titles = [t for s, _ in top_sectors for t, _, _ in by_sector.get(s, [])[:4]]
    translated = translate_batch(all_titles, src="ro", tgt="en") if all_titles else []
    t_idx = 0
    for sector, cnt in top_sectors:
        label = SECTOR_LABELS_EN.get(sector, sector.title())
        L.append(f"<h3>{label} — {cnt:,} positions</h3><ul>")
        for t, loc, sal in by_sector.get(sector, [])[:4]:
            en_t = translated[t_idx] if t_idx < len(translated) else t
            t_idx += 1
