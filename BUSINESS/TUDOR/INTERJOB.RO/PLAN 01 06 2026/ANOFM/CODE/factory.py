...
SENDER_NAME = "Elena Vasilescu - Technology Transfer Office"
REPLY_TO    = os.environ.get("REPLY_TO", "elena.manpower.dristor@gmail.com")
SUBJECT     = "Muncitori verificati, disponibili pentru productie - catalog la cerere"
GMAIL_USER         = os.environ.get("GMAIL_USER", "elena.manpower.dristor@gmail.com")
...
TEMPLATE_FILE = Path(os.environ.get("TEMPLATE_FILE", BASE / "TEMPLATES/factory_ro_ro.txt"))
...
def send_gmail(to_email, to_name, subject, body, dry_run):
    if dry_run:
        log.info(f"[DRY-RUN][GMAIL] -> {to_email} ({to_name}) | subj: {subject}")
    ...
    if _is_valid_email and not _is_valid_email(to_email)[0]:
        ...
        return False
    ok, status = sender.send_gmail(GMAIL_USER, GMAIL_APP_PASSWORD, SENDER_NAME,
                                   to_email, to_name, subject, body, reply_to=REPLY_TO)
    if ok:
        log.info(f"SENT {to_email}")
    else:
        log.error(f"FAILED {to_email} — {status}")
        if status.startswith("refused:") and _add_to_dnc:
            _add_to_dnc(to_email, reason="bounce_smtp_rejected", source="campaign_factory.py")
    return ok
...
def main():
    ...
    if not args.dry_run and not GMAIL_APP_PASSWORD:
        sys.exit("ERROR: set GMAIL_APP_PASSWORD for live send")
    ...
    for row in rows:
        ...
        ok = send_gmail(email, (row.get("company") or "").strip(), SUBJECT, body, args.dry_run)
        if ok:
            consec_fail = 0
            sent_this_run += 1
