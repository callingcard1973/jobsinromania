import sys
#!/usr/bin/env python3
"""Campanie DEFICIT_OCUPATII - constructii (Brevo API). Sender: office@buildjobs.eu"""
...
CSV_FILE     = BASE / 'DATA/constructii.csv'
SENT_FILE    = BASE / 'DATA/sent_constructii.json'
LOG_FILE     = BASE / 'LOGS/constructii.log'
SENDER       = 'office@buildjobs.eu'
SENDER_NAME  = 'InterJob - BuildJobs.eu'
BREVO_KEY    = os.environ.get('BREVO_BUILDJOBS_API_KEY', 'REDACTED')
DAILY_CAP    = 50
DELAY_S      = 480

TEMPLATE_FILE = Path(__file__).parent.parent / "TEMPLATES/template_constructii.txt"
...
def main():
    ...
    log.info(f'sector=constructii rows={len(rows)} already_today={already} eff_limit={eff_lim} dnc={len(dnc)}')
