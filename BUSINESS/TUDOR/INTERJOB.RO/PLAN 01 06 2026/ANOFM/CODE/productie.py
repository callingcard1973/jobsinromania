import sys
#!/usr/bin/env python3
"""Campanie DEFICIT_OCUPATII - productie (Brevo API). Sender: office@factoryjobs.eu"""
...
CSV_FILE     = BASE / 'DATA/productie.csv'
SENT_FILE    = BASE / 'DATA/sent_productie.json'
LOG_FILE     = BASE / 'LOGS/productie.log'
SENDER       = 'office@factoryjobs.eu'
SENDER_NAME  = 'InterJob - FactoryJobs.eu'
BREVO_KEY    = os.environ.get('BREVO_FACTORYJOBS_API_KEY', 'REDACTED')
DAILY_CAP    = 50
DELAY_S      = 480

TEMPLATE_FILE = Path(__file__).parent.parent / "TEMPLATES/template_productie.txt"
...
def main():
    ...
    log.info(f'sector=productie rows={len(rows)} already_today={already} eff_limit={eff_lim} dnc={len(dnc)}')
