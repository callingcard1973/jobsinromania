import sys
#!/usr/bin/env python3
"""Campanie DEFICIT_OCUPATII - horeca (Brevo API). Sender: office@horecaworkers2026.eu"""
...
CSV_FILE     = BASE / 'DATA/horeca.csv'
SENT_FILE    = BASE / 'DATA/sent_horeca.json'
LOG_FILE     = BASE / 'LOGS/horeca.log'
SENDER       = 'office@horecaworkers2026.eu'
SENDER_NAME  = 'InterJob - HorecaWorkers.eu'
BREVO_KEY    = os.environ.get('BREVO_HORECAWORKERS_API_KEY', 'REDACTED')
DAILY_CAP    = 50
DELAY_S      = 480

TEMPLATE_FILE = Path(__file__).parent.parent / "TEMPLATES/template_horeca.txt"
...
def main():
    ...
    log.info(f'sector=horeca rows={len(rows)} already_today={already} eff_limit={eff_lim} dnc={len(dnc)}')
