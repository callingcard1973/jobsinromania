...
### Step 5: Collect Campaign Metrics (from the unified anofm DB)
```bash
# Total emails sent (all ANOFM history)
psql -d anofm -tc "SELECT count(*) FROM send_log;"

# Sent today
psql -d anofm -tc "SELECT count(*) FROM send_log WHERE sent_at::date=CURRENT_DATE;"

# DNC size (single source of truth)
psql -d anofm -tc "SELECT count(*) FROM dnc_master;"
# Freshness check: dnc_master should refresh every 2h (refresh_dnc_master.py)
psql -d anofm -tc "SELECT max(added_at) FROM dnc_master;"  # alert if older than 3h
