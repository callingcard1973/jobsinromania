...
4. Diff laptop vs raspibig:
   `ssh tudor@192.168.100.21 "cat /opt/ACTIVE/INTERJOB/config/domains.py"` (or `plink -pw 'RASPI_PW_REDACTED'`) and compare.
...
- raspibig only via `ssh tudor@192.168.100.21` (key-based) or `plink -pw 'RASPI_PW_REDACTED'`.
