...
1. Query raspibig:
   ```
   plink -batch -pw 'RASPI_PW_REDACTED' tudor@192.168.100.21 "psql -U tudor interjob_master -A -F',' -c \"SELECT role, count(*) FROM fw_candidates GROUP BY role ORDER BY count DESC;\""
   ```
