...
1. pscp sources to raspibig `/tmp/`:
   ```
   pscp -batch -pw 'bucare' "<file>" tudor@192.168.100.21:/tmp/
   ```
...
2. Publish (order matters — parent first so children resolve parent=3152):
   ```
   plink -batch -pw 'bucare' tudor@192.168.100.21 "python3 /tmp/publish_angajatori.py"
   plink -batch -pw 'bucare' tudor@192.168.100.21 "python3 /tmp/publish_sectors.py"
   ```
...
4. Verify all 9 URLs (parent + 8 slugs) expect `http=200`:
   ```
   plink -batch -pw 'bucare' tudor@192.168.100.21 "for u in '' agricultura/ constructii/ productie/ horeca/ ingrijire/ transport/ utilaje/ management/; do printf '%-14s ' \$u; curl -sL -o /dev/null -w 'http=%{http_code} size=%{size_download}\n' https://interjob.ro/angajatori/\$u; done"
   ```
