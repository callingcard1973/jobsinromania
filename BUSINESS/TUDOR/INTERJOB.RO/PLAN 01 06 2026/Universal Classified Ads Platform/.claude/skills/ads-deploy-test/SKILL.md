...
```bash
plink -batch -pw 'RASPI_PW_REDACTED' tudor@192.168.100.21 "cd /opt/ACTIVE/classified-ads && python -m pytest tests/ -q"
plink -batch -pw 'RASPI_PW_REDACTED' tudor@192.168.100.21 "curl -s -o /dev/null -w '%{http_code}' http://localhost:8000/health"
```
Expect `200`.
