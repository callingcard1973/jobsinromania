...
### Phase 0.5: Bounce Sweep (fv-inbox-bounce-sweep)
```
fv-trader → fv-inbox-bounce-sweep skill → sterge DSN-uri, logheaza bounced
```
- Ruleaza `CODE/sweep_bounce_dsn.py` — housekeeping: gaseste Delivery Status
  Notification (failure) in cumparlegume@gmail.com, le muta in Trash, inregistreaza
  adresele bounced in `DATA/bounce_log.csv`
- AUTO, fara confirmare (nu sterge emailuri reale)
- `bounce_log.csv` se poate alimenta ulterior in DNC unificat
...
5. Bounces: N DSN-uri sterse, M destinatari noi in bounce_log
6. Probleme: erori IMAP, parsing failures, buyers fara email

## Re-runs / Partial Runs
| Comanda | Executie |
|---------|----------|
| "sweep bounces" | Phase 0.5 only |
| "re-extract offers" | Phase 2 only |
| "rebuild price book" | Phase 3 only |
| "match again" | Phase 4 only |
| "run full cycle" | Phase 0.5→1→2→3→4→5 |
