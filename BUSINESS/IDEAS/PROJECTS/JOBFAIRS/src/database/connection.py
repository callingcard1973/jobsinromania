    ...
    def get_database_info(self) -> Dict[str, Any]:
        ...
        try:
            with self.get_session() as session:
                ...
                for table in tables:
                    try:
                        # Use SQLAlchemy's table construct for safe table name handling
                        table_obj = sa.Table(table, sa.MetaData(), autoload_with=self.sqlite_engine)
                        result = session.execute(sa.select(sa.func.count()).select_from(table_obj))
...
def get_db_session(use_postgres: bool = False):
    """
    Get database session context manager.
    ...
    """
    db = get_database()
    return db.get_session(use_postgres=use_postgres)